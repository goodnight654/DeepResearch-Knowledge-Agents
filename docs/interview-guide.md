...
> 本文档帮助你在面试中完美回答关于「智能资料研究助手」的所有问题。
...
> **T (任务)**:
>
> 我负责设计并开发一个智能资料研究助手，目标是让系统既能查询企业内部资料，也能在遇到复杂开放问题时联网补充证据，最后给出可追溯来源的研究结论。

> **A (行动)**:
>
> 1. **架构设计**: 我将系统拆成两条主线：一条是本地知识库主线，负责文档解析、知识提取、本地问答和增量更新；另一条是深度研究主线，负责复杂问题的拆解、多轮检索、网页阅读、缺口判断和最终综合。
>
> 2. **流程编排**: 我把“拆题、检索、总结、判断是否继续、生成答案”做成可循环流程。系统每轮都会判断证据是否足够，如果不够就继续补查，如果够了再生成最终结论。
>
> 3. **技术攻坚**:
>    - 文档理解：支持 PDF、图片、表格等资料解析，把原始资料切成可检索的知识片段
>    - 本地检索：融合语义检索和知识图谱，既能查相似内容，也能回答实体关系类问题
>    - 联网研究：搜索外部网页后继续读取正文，清洗分块后作为证据，最终答案用 `[E1]` 这类编号引用来源
>    - 增量更新：通过文件变更检测和事件流，只更新变化部分，避免每次全量重建
...
#### 21. 为什么要在QA之外再做一个 DeepResearch Agent？

标准答法：

> QA 适合“问一个点，答一个点”，而 DeepResearch 面向“复杂开放问题”。  
> 我们在工程上把它拆开，是为了把快路径和慢路径解耦：  
> - 快路径（QA）保持低延迟和高吞吐  
> - 慢路径（DeepResearch）允许多轮检索、补证据和结构化推理  
> 这样系统既能快，也能深，不会互相拖累。

面试加分点：
- 不是“功能重复”，而是“不同问题复杂度对应不同执行策略”
- 体现你有延迟/成本意识，不会所有问题都走昂贵多轮链路

#### 22. DeepResearch 为什么设计成 plan/retrieve/summarize/gap/synthesize？
...
> 4) 识别信息缺口并决定是否继续（gap）  
> 5) 最终整合（synthesize）  
> 关键是 `gap` 这个节点，它把“继续检索还是停止”显式化，避免盲目循环。

面试追问“为什么不一个Prompt做完？”时可补充：
- 单 Prompt 难调试，失败时无法定位问题环节
- 多节点可观测，便于做节点级优化和A/B评测
- 多节点可以独立替换模型或策略，演进成本更低

#### 23. 为什么 Web Search 要可插拔且可配置？

标准答法：

> 因为企业环境并不总是允许联网，而且外部搜索质量/成本波动很大。  
> 我把 Web Search 做成可插拔 provider（duckduckgo/tavily/serpapi/disabled）：面试 Demo 默认走 duckduckgo，证明系统具备外部研究能力；内网或合规场景可以切到 disabled，只使用本地知识库。  
> 这样既能展示 DeepResearch 的外部证据召回，也保留了企业落地时的可控性。

#### 24. Run Trace 为什么要落盘？只看日志不行吗？

标准答法：

> 普通日志是“系统视角”，Run Trace 是“任务视角”。  
> 我们把一次 run 的输入、每轮事件、输出与状态绑定在一个 `run_id` 下，便于回放、复盘和评测。  
> 对 Agent 系统来说，这比散落日志更关键，因为需要按轮解释“为什么继续检索/为什么停止”。

#### 25. Trace Viewer 在面试里怎么讲？

标准答法：

> 我会先跑一个 DeepResearch 请求，拿到 `run_id`，再打开 Trace Viewer 按轮展示：  
> - plan: 如何拆题  
> - retrieve: 每轮查了什么、召回多少  
> - summarize: 中间结论是什么  
> - gap: 为什么继续或停止  
> - synthesize: 最终答案怎么来  
> 这能证明我做的是“可解释的Agent工程”，而不是黑盒Demo。
